...
const WorkflowEngine = {
  ...
  async run(workflow, options = {}) {
    ...
    if (window.executionTracer) {
      window.executionTracer.startTrace(workflow.id || workflow.name);
    }
    ...
    try {
      ...
      while (queue.length > 0 && this.isRunning) {
        ...
        this.publish('nodeStart', { nodeId, node, variables });
        this.log(`执行节点: ${node.type} - ${node.subtype}`);

        // 检查断点（由外部 debugger 处理，这里仅发布事件）
        this.publish('beforeNodeExecute', { nodeId, node, variables });
        ...
        // 更新变量（模拟）
        if (node.config && node.config.outputVar) {
          variables[node.config.outputVar] = { nodeId, output: null };
        }

        const result = await executor(node, { outputs, variables });
        ...
        if (result.output) {
          Object.entries(result.output).forEach(([key, value]) => {
            ...
            if (window.variableInspector) {
              window.variableInspector.setVariable(key, value, typeof value);
            }
          });
        }
        ...
        // 记录执行轨迹
        if (window.executionTracer) {
          window.executionTracer.recordStep(nodeId, variables, result, result.success ? 'completed' : 'error');
        }

        // 拍摄时间旅行快照
        if (window.timeTravel && window.executionTracer) {
          window.timeTravel.createSnapshotFromTrace(`Step ${nodeId}`);
        }

        this.publish('nodeComplete', { nodeId, result, variables });
      ...
      this.log('工作流执行完成');
      this.publish('complete', { outputs, variables });
      
      // 停止跟踪
      if (window.executionTracer) {
        window.executionTracer.stopTrace();
      }
    } catch (error) {
